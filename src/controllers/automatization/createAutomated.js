...
const createAutoTransaction = async (req, res) => {
  try {
    ...
    if (req.body.type === 'expense') {
      ...
      if (automatization.automatized) return res.status(201).json({ success: true, message: 'automatized expense created!' });
    }
    else if (req.body.type === 'income') {
      const automatization = await createIncomeAutomatization(req.body, id);
      if (automatization.automatized) return res.status(201).json({ success: true, message: 'automatized income created!' });
    }
